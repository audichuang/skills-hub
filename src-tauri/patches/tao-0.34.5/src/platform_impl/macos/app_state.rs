...
impl AppState {
  ...
  pub fn launched(app_delegate: &Object) {
    ...
    unsafe {
      ...
      // Use `activate` (available since macOS 14) instead of the deprecated
      // `activateIgnoringOtherApps` which was removed in macOS 26 and causes
      // a panic (abort) when called inside the extern "C" FFI callback.
